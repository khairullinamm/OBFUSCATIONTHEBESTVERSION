...
void CreateVariables(FILE* Read)
{
	...
	while (!feof(Read))
	{
		...
		if (strstr(GetWord, "void") || strstr(GetWord, "int") || strstr(GetWord, "char") || strstr(GetWord, "float") || strstr(GetWord, "FILE"))
		{
			char s1;
		MarkInside:
		//сюда заходим только в том случае, если GetWord - включает в себя тип функции
			s1 = '0';
			int i = 0;
			//isspace - проверяет наличие пробела 
			while (isspace(s1 = fgetc(Read)) || s1 == '*') //доходим до начала названия функции
				i++;
			//isalpha - проверяет является ли буква
			if ((isalpha(s1) || s1 == '_') && (i != 0))
			{
				...
				находим открытую или закрытую скобку или запятую или точку с запятой
				while (strchr("(),;{}", s1) == NULL)
				{
					s1 = fgetc(Read);
				}

				if (s1 == '(') FlagFunction = 1; //находимся в объявлении переменных, передаваемых в функции
				else if (s1 == ')') FlagFunction = 0; // не находимся в объявлении переменных, передаваемых в функции
	...
	for (int i = 0; i < LinesCount - 1; i++)
	{
		...
		for (int j = 0; j < Length; j++) //переименовываем названия функций и переменных
		{
			unsigned char c;
			if (j % 3 == 0) 
				c = rand() % 25 + 65;
