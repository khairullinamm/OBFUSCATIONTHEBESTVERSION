	...
	while (!feof(Read))
	{
		...
		//вне объявлении переменных
			...
			if ((isalpha(s1) || s1 == '_') && (i != 0))
			{
				...
				else if (s1 == ',' && FlagFunction == 1) goto MarkOutside; //запятая в объявлении переменных
				else if (s1 == ',' && FlagFunction == 0) goto MarkInside; //запятая не в объявлении переменных
				else if (s1 == '{')
					while ((s1 = fgetc(Read)) != '}');
			}
		}
